...
import { CipherSuite, LeafNodeSource, ProtocolVersion, ResumptionPSKUsage, WireFormat } from "../src/types.ts";
...
Deno.test("RatchetTree - Basic operations", () => {
  // Initialize a tree
  const tree = new RatchetTree(CipherSuite.MLS_128_DHKEMX25519_AES128GCM_SHA256_Ed25519);
  assertEquals(tree.leafCount, 0);
  assertEquals(tree.depth, 0);
  
  // Test leaf to node conversion
  const leafIndex1 = 0;
  const nodeIndex1 = RatchetTree.leafToNode(leafIndex1);
  assertEquals(nodeIndex1, 0);
  
  const leafIndex2 = 5;
  const nodeIndex2 = RatchetTree.leafToNode(leafIndex2);
  assertEquals(nodeIndex2, 10);
  ...
  assertEquals(RatchetTree.parent(3), 7);
  assertEquals(RatchetTree.leftChild(7), 3);
  assertEquals(RatchetTree.rightChild(7), 11);
  assertEquals(RatchetTree.sibling(3), 11);
  assertEquals(RatchetTree.sibling(11), 3);
  
  // Test level calculation
  assertEquals(RatchetTree.level(0), 0); // Leaf
  assertEquals(RatchetTree.level(3), 0); // Leaf
  assertEquals(RatchetTree.level(2), 1); // Parent
  assertEquals(RatchetTree.level(7), 2); // Higher parent
});
...
Deno.test("RatchetTree - Path computation", () => {
  ...
  assertEquals(copath0[0], 2); // Sibling of parent of leaf 0
  assertEquals(copath0[1], 7); // Sibling of root
  
  const copath1 = tree.copath(1); 
  assertEquals(copath1.length, 2);
  assertEquals(copath1[0], 0); // Sibling of leaf 1
  assertEquals(copath1[1], 7); // Sibling of root
});
...
Deno.test("MLSGroup - Group resumption", async () => {
  ...
  assertEquals(resumedGroup.getEpoch(), 1n); // We did 1 commit to add members
