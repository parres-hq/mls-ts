...
Deno.test("RatchetTree - Path computation", () => {
  ...
  const leaf = {
    ...
    lifetime: { notBefore: 0n, notAfter: BigInt(Date.now() + 24 * 60 * 60 * 1000) }, // 24 hours validity
    extensions: [],
...
Deno.test("RatchetTree - Tree hash", () => {
  ...
  const leaf = {
    ...
    leafNodeSource: LeafNodeSource.KEY_PACKAGE,
    lifetime: { notBefore: 0n, notAfter: BigInt(Date.now() + 24 * 60 * 60 * 1000) }, // Add lifetime
    extensions: [],
